# PLAN.md — 基金交易系统（Telegram 核心）冲刺行动计划（多模型并行版）

> 本计划用于你在 >=2 个 tmux 窗口中同时运行 Claude Code（接 GLM4.7）与 Kimi Code（接 Kimi K2.5）时，避免重复劳动、相互覆盖、跑偏需求，并保证交付可验收、可回滚。  
> 该计划与《基金交易系统_最终方案_修订版.md》一致，后者为“架构与契约真相源”。任何冲突以架构文档为准。
